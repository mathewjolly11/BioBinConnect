...
class BioBinValidation {
    ...
    setupPhoneValidation() {
        const phoneInputs = document.querySelectorAll('input[placeholder*="phone"], input[placeholder*="Phone"], input[placeholder*="mobile"], input[placeholder*="Mobile"]');
    ...
    validatePhone(input) {
        ...
        if (phone.charAt(0) < '6') {
            this.showValidationError(input, 'Phone number should start with 6, 7, 8, or 9');
            return false;
        }
    ...
    validateName(input) {
        ...
        if (!nameRegex.test(name)) {
            this.showValidationError(input, 'Name should only contain letters and spaces');
            return false;
        }
    ...
    setupNumberValidation() {
        const numberInputs = document.querySelectorAll('input[type="number"]:not(.house-number-input)');
    ...
    validateFile(input) {
        ...
        if (!allowedTypes.includes(file.type)) {
            this.showValidationError(input, 'Please select a valid image file (JPEG, PNG, GIF, WebP, AVIF)');
            return false;
        }
    ...
    setupHouseNumberValidation() {
        const houseInputs = document.querySelectorAll('.house-number-input, input[data-validate-house="true"]');
        ...
        houseInputs.forEach(input => {
            ...
            input.addEventListener('input', (e) => {
                ...
                if (value) {
                    this.showValidationSpinner(input);
                    timeout = setTimeout(() => {
    ...
    async validateHouseNumber(input) {
        ...
        const raSelect = form.querySelector('select[name*="residents_association"]');
        ...
        const data = {
            ...
            current_household_id: form.querySelector('input[name="current_household_id"]')?.value || null
        };
        
        try {
            const response = await fetch('/guest/api/validate-house-number/', {
                method: 'POST',
                headers: {
            ...
            this.showValidationError(input, 'Unable to validate house number. Please try again.');
            return false;
        }
    }
    ...
    createPasswordToggle(input) {
        ...
        toggleBtn.addEventListener('click', () => {
            ...
            toggleBtn.title = isPassword ? 'Click to hide password' : 'Click to show password';
        });
    ...
    clearValidationMessage(input) {
        ...
        if (container) {
            ...
            while (nextElement && (nextElement.classList?.contains('error-message') || nextElement.classList?.contains('success-message') || nextElement.classList?.contains('validation-message'))) {
                const toRemove = nextElement;
                nextElement = nextElement.nextSibling;
                toRemove.remove();
            }
        } else {
            const messages = input.parentNode.querySelectorAll('.error-message, .success-message, .validation-message');
    ...
    setupPasswordValidation() {
        ...
        passwordInputs.forEach(input => {
            ...
            if (input.name === 'password2' || input.placeholder.toLowerCase().includes('confirm')) {
                input.addEventListener('input', (e) => {
    ...
    setupRealTimeValidation() {
        ...
        forms.forEach(form => {
            // Skip validation for login form - let Django handle it
            const isLoginForm = form.querySelector('button[value="login"]') || 
                               form.action.includes('/login/') || 
                               form.querySelector('input[placeholder*="Email"]') && form.querySelector('input[placeholder*="Password"]') && form.querySelectorAll('input').length <= 3;
            
            // Also skip validation for registration form - let Django handle it
    ...
    validateForm(form) {
        ...
        inputs.forEach(input => {
            ...
            // Also check if the field is in a hidden role container
            const roleContainer = input.closest('.role-fields');
            const isInHiddenRole = roleContainer && getComputedStyle(roleContainer).display === 'none';
            
            if (!isVisible || isInHiddenRole) {
            ...
            } else if (input.getAttribute('placeholder')?.toLowerCase().includes('phone') || 
                      input.getAttribute('placeholder')?.toLowerCase().includes('mobile')) {
                fieldValid = this.validatePhone(input);
            } else if (input.getAttribute('placeholder')?.toLowerCase().includes('name')) {
            ...
            if (!fieldValid) {
                isValid = false;
                console.log('Validation failed for field:', input.name || input.placeholder, 'Value:', input.value, 'Type:', input.type);
            }
        });
    ...
    showValidationSpinner(input) {
        ...
        spinnerDiv.innerHTML = 'Checking availability <span class="validation-spinner"></span>';
    ...
    clearValidationMessage(input) {
        ...
        if (container) {
            ...
            const orphanedMessages = parent.querySelectorAll('.error-message, .success-message, .validation-message');
            orphanedMessages.forEach(msg => {
                // Only remove if it's not attached to another input
                if (!msg.previousElementSibling || msg.previousElementSibling === input || 
